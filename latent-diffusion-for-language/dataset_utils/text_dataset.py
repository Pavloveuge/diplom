...
from datasets import load_dataset, Value, Dataset as HFDataset
...
def get_dataset(dataset_name, metadata=False, synthetic_train_path=None):
    if dataset_name == 'roc':
        ...
        def preproc_text(text):
            ...
            return text
        manifest_path = "/diplom/diplom/train-100-manifest.json"
        with open(manifest_path) as f:
            manifest = [json.loads(line) for line in f]
        dataset = HFDataset.from_list(manifest)
        dataset = dataset.map(preproc_text)
        dataset = dataset.remove_columns(['audio_filepath', 'duration'])
